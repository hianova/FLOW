...
#define CHECK(cond) if (!(cond)) { fprintf(stderr, "hetero-swarm-mesh-test failed at %s:%d: %s\n", __FILE__, __LINE__, #cond); exit(1); }

int main(void) {
    printf("========================================================================================\n");
    printf("  🐝 Running FLOW Heterogeneous Pheromone Mesh & Distributed DAG Suite\n");
    printf("========================================================================================\n\n");

    /* ---------------------------------------------------------------------------------- */
    /* STAGE 1: Heterogeneous Multi-Role Node Registration in Federated Mesh              */
    /* ---------------------------------------------------------------------------------- */
    printf("--- [Stage 1: Heterogeneous Multi-Role Mesh Registration] ---\n");
    FlowHeteroMesh mesh;
    flow_hetero_mesh_init(&mesh);
    CHECK(mesh.node_count == 0);

    /* Register Ingress Gateway (HTTP/3), Two Compute Workers, and Storage Index */
    CHECK(flow_hetero_mesh_register_node(&mesh, 1, FLOW_SWARM_ROLE_INGRESS_GATEWAY, "edge_ingress_quic", 0x1111, 50000));
    CHECK(flow_hetero_mesh_register_node(&mesh, 2, FLOW_SWARM_ROLE_COMPUTE_ROUTER,  "worker_compute_a",   0x2222, 30000));
    CHECK(flow_hetero_mesh_register_node(&mesh, 3, FLOW_SWARM_ROLE_COMPUTE_ROUTER,  "worker_compute_b",   0x3333, 30000));
    CHECK(flow_hetero_mesh_register_node(&mesh, 4, FLOW_SWARM_ROLE_STORAGE_INDEX,   "sharded_vault_db",   0x4444, 70000));
    CHECK(mesh.node_count == 4);
    ...
    /* ---------------------------------------------------------------------------------- */
    printf("--- [Stage 2: 9-Byte Heterogeneous Pheromone Packet Bit-Exact Serialization] ---\n");
    ...
    CHECK(flow_swarm_hetero_encode(&tx_pkt, wire_packet) == 1);
    CHECK(wire_packet[0] == FLOW_SWARM_MSG_HETERO_PHEROMONE); /* 0xBB */

    FlowHeteroPheromonePacket rx_pkt;
    CHECK(flow_swarm_hetero_decode(wire_packet, &rx_pkt) == 1);
    CHECK(rx_pkt.role == FLOW_SWARM_ROLE_COMPUTE_ROUTER);
    CHECK(rx_pkt.node_id == 2);
    CHECK(rx_pkt.backpressure_permille == 850);
    CHECK(rx_pkt.latency_p99_us == 1200);
    CHECK(rx_pkt.contract_crc16 == 0x42F1);
    ...
    /* ---------------------------------------------------------------------------------- */
    printf("--- [Stage 3: Dynamic Fluid Routing via Downstream Pheromone Gradients] ---\n");

    /* Telemetry Broadcast: Node 2 (Compute A) is congested; Node 3 (Compute B) is cool */
    CHECK(flow_hetero_mesh_receive_pheromone(&mesh, wire_packet) == 1);
    ...
    CHECK(flow_swarm_hetero_encode(&cool_pkt, cool_wire) == 1);
    CHECK(flow_hetero_mesh_receive_pheromone(&mesh, cool_wire) == 1);

    /* Ingress Gateway chooses downstream target for Compute tier */
    uint8_t selected_id = 0;
    CHECK(flow_hetero_mesh_route_target(&mesh, FLOW_SWARM_ROLE_COMPUTE_ROUTER, &selected_id) == 1);
    CHECK(selected_id == 3); /* Must select Node 3 because cost(65us * 1.5) << cost(1200us * 5.25) */
    ...
    /* ---------------------------------------------------------------------------------- */
    printf("--- [Stage 4: 1-Bit Chaos Subspace Concurrency Auto-Throttling] ---\n");
    ...
    /* ---------------------------------------------------------------------------------- */
    printf("--- [Stage 5: SMT Formal Flow Conservation & Equilibrium Proofs] ---\n");
    ...
    CHECK(strstr(proof.proof_summary, "exceeds pipeline bottleneck tier capacity") != NULL);
    printf("  ✓ SMT Overload Guard Caught: 90k QPS > 60k QPS correctly rejected with SAT counterexample: '%s'\n\n", proof.proof_summary);

    printf("========================================================================================\n");
    printf("  HETERO_SWARM_MESH_TEST=PASSED: 9-byte fluid mesh sound, smt-safe, and self-healing!\n");
    printf("========================================================================================\n");
    return 0;
}
