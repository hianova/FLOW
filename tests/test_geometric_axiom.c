...
int main(void) {
    ...
    FLOW_STAGE_BEGIN(1, "Hardware Alignment & Fiber Bundle Invariant");
    {
        ...
        FlowUnifiedSection *sec = flow_axiom_create("TEST_ALIGN");
        FLOW_ASSERT_TRUE(sec != NULL);
        FLOW_ASSERT_EQ((uintptr_t)sec % 64, 0);
        printf("    * FlowUnifiedSection size = %zu bytes (%zu cache lines), alignof = %zu, heap_align = 0x%lx\n",
               sizeof(FlowUnifiedSection), sizeof(FlowUnifiedSection) / 64, alignof(FlowUnifiedSection), (uintptr_t)sec % 64);
    ...
    {
        FlowUnifiedSection *sec = flow_axiom_create("HFT_ROBOTIC_CONTROL");
        FLOW_ASSERT_TRUE(sec != NULL);
    ...
    {
        FlowUnifiedSection *sec = flow_axiom_create("TEST_TRANSVERSALITY");
        FLOW_ASSERT_TRUE(sec != NULL);
    ...
    {
        FlowUnifiedSection *sec = flow_axiom_create("THERMAL_CONTACT_RUN");
        FLOW_ASSERT_TRUE(sec != NULL);
    ...
    {
        FlowUnifiedSection *src_sec = flow_axiom_create("MORPH_SOURCE");
        FLOW_ASSERT_TRUE(src_sec != NULL);
        src_sec->q[0] = 42.0;
        src_sec->p[0] = 3.14;
        src_sec->s = 99.9;
        ...
        /* Morph Jet -> Section */
        FlowUnifiedSection *roundtrip_sec = flow_axiom_create("MORPH_TARGET");
        FLOW_ASSERT_TRUE(roundtrip_sec != NULL);
        ...
        flow_axiom_destroy(src_sec);
        flow_axiom_destroy(roundtrip_sec);
    }
    ...
    {
        FlowUnifiedSection *sec = flow_axiom_create("FORMAL_PROOF_CHECK");
        FLOW_ASSERT_TRUE(sec != NULL);
