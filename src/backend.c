...
#ifndef FLOW_TEMPLATE_PATH
#define FLOW_TEMPLATE_PATH "tools/self-host-stage2.c"
#endif
...
static void emit_base64_source(FILE *output, FILE *source) {
    static const char alphabet[] =
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    unsigned char bytes[3];
    size_t count;

    fputs("static const char *self_template_b64[] = {\n", output);
    while ((count = fread(bytes, 1, sizeof(bytes), source)) > 0) {
        unsigned char encoded[4];
        size_t i;
        encoded[0] = alphabet[bytes[0] >> 2];
        encoded[1] = alphabet[((bytes[0] & 3u) << 4) |
                              (count > 1 ? bytes[1] >> 4 : 0u)];
        encoded[2] = count > 1 ? alphabet[((bytes[1] & 15u) << 2) |
                                          (count > 2 ? bytes[2] >> 6 : 0u)] : '=';
        encoded[3] = count > 2 ? alphabet[bytes[2] & 63u] : '=';
        fputc('"', output);
        for (i = 0; i < sizeof(encoded); ++i) fputc(encoded[i], output);
        fputs("\",\n", output);
    }
    fputs("    NULL\n};\n\n", output);
}
...
static int emit_flowc_bootstrap_v2(FILE *output, const SemanticIR *ir) {
    FILE *template = fopen(FLOW_TEMPLATE_PATH, "rb");
    ...
    fputs("/* Standalone FLOW compiler bootstrap. Selection is table-driven from facts. */\n"
          ...
          "    int compiler_domain;\n"
          "} SelfSpec;\n\n", output);
    emit_base64_source(output, template);
    fclose(template);
    fputs("static int b64_value(char character) {\n"
          "    const char *alphabet = \"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/\";\n"
          "    const char *found = strchr(alphabet, character);\n"
          "    return found == NULL ? -1 : (int)(found - alphabet);\n"
          "}\n\n"
          "static int emit_template(FILE *output) {\n"
          "    unsigned long value = 0;\n"
          "    unsigned bits = 0;\n", output);
    fputs("    size_t part;\n"
          "    size_t i;\n"
          "    for (part = 0; self_template_b64[part] != NULL; ++part)\n"
          "        for (i = 0; self_template_b64[part][i] != '\\0'; ++i) {\n"
          "            int digit = b64_value(self_template_b64[part][i]);\n"
          "            if (digit < 0) continue;\n"
          "            value = (value << 6) | (unsigned)digit;\n"
          "            bits += 6;\n"
          "            if (bits >= 8) {\n"
          "                bits -= 8;\n"
          "                if (fputc((int)((value >> bits) & 255u), output) == EOF) return 0;\n"
          "            }\n"
          "        }\n"
          "    return ferror(output) == 0;\n"
          "}\n\n", output);
    ...
    fputs("typedef struct { const char *name; int shared; int unordered; int parallel; const char *binding; int score; } SelfComponent;\n"
          ...
          "    return best;\n"
          "}\n\n", output);
    fputs("static int is_compiler_spec(FILE *input) {\n"
          "    char line[512]; int compiler_domain = 0; int compiler_contract = 0;\n"
          "    while (fgets(line, sizeof(line), input) != NULL) {\n"
          "        if (strncmp(line, \"domain compiler\", 15) == 0) compiler_domain = 1;\n"
          "        if (strncmp(line, \"contract compiler\", 17) == 0) compiler_contract = 1;\n"
          "    }\n"
          "    return compiler_domain && compiler_contract;\n"
          "}\n\n", output);
    fputs("static int emit_target(FILE *output, const SelfSpec *spec) {\n"
          ...
          "    if (is_compiler_spec(input)) { fclose(input); output = fopen(argv[3], \"w\"); if (output == NULL) return EXIT_FAILURE; ok = emit_template(output); if (fclose(output) != 0) ok = 0; return ok ? EXIT_SUCCESS : EXIT_FAILURE; }\n"
          "    rewind(input);\n"
