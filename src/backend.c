...
static void emit_candidate_plan(FILE *output, const SemanticIR *ir) {
    ...
    /* Compiler-Guided Symplectic Shaping: Polyhedral Synthesized Hamiltonian */
    size_t cap = ir->input_max_count > 0 ? ir->input_max_count : 64;
    double cap_d = (double)cap;
    double omega_base = sqrt(cap_d) / 16.0;
    if (omega_base < 0.1) omega_base = 0.1;
    double mu = 1.0 / cap_d;
    double q_sat = 1.25 * cap_d;
    ...
    fprintf(output,
        ...
        "    double q_saturation;\n"
        "    double moreau_kappa;\n"
        "} FlowJetSynthesizedLandscape;\n\n"
        "static const FlowJetSynthesizedLandscape FLOW_JET_SYNTHESIZED_LANDSCAPE __attribute__((unused)) = {\n"
        "    .dim = 16,\n"
        "    .omega = %.6f,\n"
        "    .barrier_mu = %.8f,\n"
        "    .q_saturation = %.6f,\n"
