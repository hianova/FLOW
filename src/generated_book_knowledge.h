...
static const FlowBookChapterDoc FLOW_BOOK_CHAPTERS_ZH[FLOW_BOOK_CHAPTER_COUNT] = {
    ...
    },
    {
        ...
        .chapter_title = "第五章：Floquet 離散時間晶體動力學 (DTC 次諧波 2T 鎖定、極限環相角儲存與 CPG 節律)",
        .philosophy_why = "工程的嚴謹在於承認物理邊界。FLOW 借鑒 Floquet 時間平移對稱破缺理論，在相空間構建高抗噪的非線性極限環動力學原型。",
        .book_excerpt = "Floquet 週期驅動誘導離散時間平移對稱破缺，形成次諧波 2T 剛性鎖定 (100% 實測比率)。在雙穩態極限環中無耗散保持相角位元，為具身 CPG 步態與高抖動傳感器軟體鎖相提供動力學原語。"
    },
    {
        .chapter_ref = "ch06_smt_formal_verification.md",
        .chapter_title = "第六章：形式化邊界驗證 (零依賴 SMT 超盒多面體驗證、SMT-LIB2 導出與死碼消除)",
        .philosophy_why = "啟發式搜尋可以天馬行空，但發射出的每一行機器碼必須擁有無可爭辯的邊界保證。FLOW 採用零外部依賴超盒多面體約束檢驗，既杜絕運行期越界，又避免引入數十 MB 龐大求解器。",
        .book_excerpt = "SMT 超盒驗證器以納秒級速度 (~115ns) 驗證緩衝區邊界、記憶體配額上限、分片隔離與確定性 4 大定理，並支援導出標準 SMT-LIB2 腳本對接外部 Z3 求解器；在編譯期消除下游冗餘防禦分支。"
    },
    {
    ...
    },
    {
        ...
        .chapter_title = "第八章：六大數學支柱：經驗啟發式的全息消解與納秒級工程降維",
        .philosophy_why = "凡充斥經驗常數與魔術數字之處，皆是脆弱的避難所。FLOW 以古典控制論與離散數學工具消解工程猜測，並降維為數十納秒內的 C 原語，兼顧理論嚴密與硬體實時性。",
        .book_excerpt = "多面體模型約束整數邊界，在線凸最佳化 (OCO) 次梯度調整快取影子價格，Lyapunov 隊列流體漂移判據限流，勢能博弈以 BPR 擁塞函數貪心路由，Moreau 過程降維為 14ns 施密特遲滯環，代數同調論以並查集+歐拉公式 14ns 計算環洞。"
    },
    {
    ...
    },
    {
        ...
        .chapter_title = "第十一章：JIT 代碼發射與記憶體幾何變形 (AoS 到 SoA 即時重映射與生存模式)",
        .philosophy_why = "程式碼不是雕刻在石頭上的死文字，而是能在記憶體中靈活轉置的數據結構。當記憶體壓力逼近臨界時，系統在微秒內完成拓樸幾何變形，安全退守靜態避難所。",
        .book_excerpt = "JIT 發射器支援 C、Rust、Python 多目標發射。透過內聯跨步迴圈實現 AoS 與 SoA 數據轉置 (單字節 ~0.2ns) 與列式零拷貝引用分享；在極端低記憶體下自我否決 JIT，遁入零分配 Static Survival 避難所。"
    },
    {
