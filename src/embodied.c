...
int flow_embodied_encode_genome(const FlowEmbodiedCoordinate *coord, uint64_t *genome_out) {
    if (coord == NULL || genome_out == NULL) return 0;
    uint64_t g = FLOW_GENOME_PACK((uint64_t)coord->gait_mode, FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT)
               | FLOW_GENOME_PACK((uint64_t)coord->torque_gain_scale, FLOW_EMBODIED_OFF_TORQUE, FLOW_EMBODIED_LEN_TORQUE)
               | FLOW_GENOME_PACK((uint64_t)coord->sensor_confidence, FLOW_EMBODIED_OFF_SENSOR, FLOW_EMBODIED_LEN_SENSOR)
               | FLOW_GENOME_PACK((uint64_t)coord->thermal_throttle_level, FLOW_EMBODIED_OFF_THERMAL, FLOW_EMBODIED_LEN_THERMAL)
               | FLOW_GENOME_PACK((uint64_t)coord->smith_delay_steps, FLOW_EMBODIED_OFF_SMITH, FLOW_EMBODIED_LEN_SMITH)
               | FLOW_GENOME_PACK((uint64_t)coord->fleet_clearance_mm, FLOW_EMBODIED_OFF_FLEET, FLOW_EMBODIED_LEN_FLEET)
               | FLOW_GENOME_PACK((uint64_t)coord->survival_flags, FLOW_EMBODIED_OFF_SURVIVAL, FLOW_EMBODIED_LEN_SURVIVAL);
    *genome_out = g;
    return 1;
}

int flow_embodied_decode_genome(uint64_t genome, FlowEmbodiedCoordinate *coord_out) {
    if (coord_out == NULL) return 0;
    coord_out->gait_mode = (FlowCorticalGaitMode)FLOW_GENOME_GET(genome, FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT);
    coord_out->torque_gain_scale = (uint8_t)FLOW_GENOME_GET(genome, FLOW_EMBODIED_OFF_TORQUE, FLOW_EMBODIED_LEN_TORQUE);
    coord_out->sensor_confidence = (uint8_t)FLOW_GENOME_GET(genome, FLOW_EMBODIED_OFF_SENSOR, FLOW_EMBODIED_LEN_SENSOR);
    coord_out->thermal_throttle_level = (uint8_t)FLOW_GENOME_GET(genome, FLOW_EMBODIED_OFF_THERMAL, FLOW_EMBODIED_LEN_THERMAL);
    coord_out->smith_delay_steps = (uint8_t)FLOW_GENOME_GET(genome, FLOW_EMBODIED_OFF_SMITH, FLOW_EMBODIED_LEN_SMITH);
    coord_out->fleet_clearance_mm = (uint16_t)FLOW_GENOME_GET(genome, FLOW_EMBODIED_OFF_FLEET, FLOW_EMBODIED_LEN_FLEET);
    coord_out->survival_flags = (uint16_t)FLOW_GENOME_GET(genome, FLOW_EMBODIED_OFF_SURVIVAL, FLOW_EMBODIED_LEN_SURVIVAL);
    return 1;
}
...
int flow_embodied_superpose_safety_canvas(const FlowPhysicsEngine *phys,
    ...
    /* Valid dynamic gaits: FLAT_WALK (bit 0=1), STAIR_CLIMB (bit 1=2), ROUGH_TERRAIN (bit 2=4) */
    const uint64_t dynamic_gaits_init = 1ULL | 2ULL | 4ULL; /* 0x07 */
    const uint64_t base_mask = (~FLOW_GENOME_FIELD_MASK(FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT))
                             | FLOW_GENOME_PACK(dynamic_gaits_init, FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT);

    canvas_out->zmp_mask = base_mask;
    canvas_out->kalman_mask = base_mask;
    canvas_out->thermal_mask = base_mask;
    canvas_out->fleet_mask = base_mask;

    /* 1. Physics & ZMP Dynamic Stability Mask */
    if (phys != NULL) {
        if (!flow_physics_is_zmp_stable(&phys->current_state)) {
            /* ZMP outside polygon: forbid high dynamic gaits (STAIR_CLIMB: bit 1=2 & ROUGH_TERRAIN: bit 2=4 -> 6) */
            uint64_t forbidden_gaits = FLOW_GENOME_PACK(2ULL | 4ULL, FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT);
            canvas_out->zmp_mask &= ~forbidden_gaits;
            /* Clamp torque scale to 50% max */
            canvas_out->zmp_mask &= ~FLOW_GENOME_PACK(0xC0, FLOW_EMBODIED_OFF_TORQUE, FLOW_EMBODIED_LEN_TORQUE);
        }
        if (phys->violations_prevented_total > 0) {
            /* Motor burnout risk: clear high torque bits */
            canvas_out->zmp_mask &= ~FLOW_GENOME_PACK(0x80, FLOW_EMBODIED_OFF_TORQUE, FLOW_EMBODIED_LEN_TORQUE);
        }
    }

    /* 2. Sensor Fusion & Kalman Noise Level Mask */
    if (fusion != NULL) {
        if (fusion->sensor_confidence < 0.5 || fusion->raw_vibration_noise_level > 0.05) {
            /* High sensor noise: mask out rough terrain gait (ROUGH_TERRAIN: bit 2=4) */
            uint64_t forbidden_rough = FLOW_GENOME_PACK(4ULL, FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT);
            canvas_out->kalman_mask &= ~forbidden_rough;
        }
    }

    /* 3. Thermodynamic Energy & Battery Throttling Mask */
    if (gov != NULL) {
        if (gov->edge_chip_temp_celsius >= gov->thermal_throttle_limit_celsius ||
            gov->current_battery_percent < 15.0) {
            /* Thermal throttle: forbid high torque and sprint gaits (FLAT_WALK: bit 0=1, STAIR_CLIMB: bit 1=2 -> 3) */
            canvas_out->thermal_mask &= ~FLOW_GENOME_PACK(0xF0, FLOW_EMBODIED_OFF_TORQUE, FLOW_EMBODIED_LEN_TORQUE);
            canvas_out->thermal_mask &= ~FLOW_GENOME_PACK(1ULL | 2ULL, FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT);
        }
    }

    /* 4. Swarm Fleet Spatial Collision Polytope Mask */
    if (fleet != NULL && fleet->robot_count > 1) {
        int proximity_warning = 0;
        for (size_t i = 0; i < fleet->robot_count; ++i) {
            for (size_t j = i + 1; j < fleet->robot_count; ++j) {
                double dx = fleet->robots[i].position[0] - fleet->robots[j].position[0];
                double dy = fleet->robots[i].position[1] - fleet->robots[j].position[1];
                double dist = sqrt(dx * dx + dy * dy);
                if (dist < fleet->min_safety_margin_m) {
                    proximity_warning = 1;
                    break;
                }
            }
            if (proximity_warning) break;
        }
        if (proximity_warning || fleet->total_collision_avoidance_interventions > 0) {
            /* Proximity hazard: forbid forward high-speed walking (FLAT_WALK: bit 0=1) */
            canvas_out->fleet_mask &= ~FLOW_GENOME_PACK(1ULL, FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT);
        }
    }

    /* 5. Tensor-Product Hypergeometric Superposition */
    canvas_out->superposed_mask = canvas_out->zmp_mask
                                & canvas_out->kalman_mask
                                & canvas_out->thermal_mask
                                & canvas_out->fleet_mask;

    /* Check Double-Bind Condition: if all active dynamic gaits are masked out */
    uint64_t allowed_gaits = FLOW_GENOME_GET(canvas_out->superposed_mask, FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT);
    if (canvas_out->superposed_mask == 0 || allowed_gaits == 0) {
        canvas_out->is_double_bind = 1;
        /* Immediately engage zero-downtime Static Survival Brace fallback */
        canvas_out->superposed_mask = FLOW_GENOME_PACK(FLOW_GAIT_EMERGENCY_BRACE, FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT)
                                    | FLOW_GENOME_PACK(1, FLOW_EMBODIED_OFF_SURVIVAL, FLOW_EMBODIED_LEN_SURVIVAL);
    } else {
        canvas_out->is_double_bind = 0;
    }

    /* Soft dynamic bias: prefer nominal flat walk when legal */
    canvas_out->dynamic_bias = FLOW_GENOME_PACK(FLOW_GAIT_FLAT_WALK, FLOW_EMBODIED_OFF_GAIT, FLOW_EMBODIED_LEN_GAIT)
                             | FLOW_GENOME_PACK(128, FLOW_EMBODIED_OFF_TORQUE, FLOW_EMBODIED_LEN_TORQUE);
...
int flow_embodied_step_token_ring(FlowEmbodiedCanvas *canvas,
    ...
    if (canvas == NULL || current_genome_inout == NULL) return 0;

    if (canvas->is_double_bind) {
        /* Under Double-Bind, hard lock to static survival brace */
        *current_genome_inout = canvas->superposed_mask;
        if (mutated_bit_out) *mutated_bit_out = FLOW_EMBODIED_OFF_GAIT + 2;
        return 1;
    }

    /* 1. Discrete Attention Operator: Canvas projection */
    uint64_t projected = flow_manifold_project(*current_genome_inout, canvas->superposed_mask, canvas->dynamic_bias);

    /* 2. 1-Bit Chaotic Manifold Transition */
    uint64_t next_genome = flow_manifold_transition(projected, canvas->superposed_mask, rng_state_inout, mutated_bit_out);

    *current_genome_inout = next_genome;
    return 1;
}
...
FlowSMTResult flow_embodied_verify_smt(const FlowRigidBodyState *state,
    ...
    /* Theorem 1: ZMP & Joint Torque Safety Polytope */
    uint64_t zmp_violation = 0;
    if (state != NULL) {
        if (!flow_physics_is_zmp_stable(state)) zmp_violation = 1;
        for (size_t i = 0; i < state->joint_count && i < FLOW_MAX_JOINTS; ++i) {
            if (fabs(state->joint_torques[i]) > state->max_torque_limit[i]) {
                zmp_violation = 2;
                break;
            }
        }
    }
    FLOW_SMT_BOX_ADD_RULE(builder, "zmp and torque violation", zmp_violation, 0, 0,
                          FLOW_BOX_THEOREM_BUFFER_BOUNDS, "Center of Mass or Torque exceeds physical rating");

    /* Theorem 2: Thermal and Battery Energy Quota Bound */
    uint64_t thermal_violation = 0;
    if (gov != NULL) {
        if (gov->edge_chip_temp_celsius > gov->thermal_throttle_limit_celsius) {
            thermal_violation = (uint64_t)(gov->edge_chip_temp_celsius - gov->thermal_throttle_limit_celsius);
        }
    }
    FLOW_SMT_BOX_ADD_RULE(builder, "thermal excess", thermal_violation, 0, 0,
                          FLOW_BOX_THEOREM_MEMORY_QUOTA, "Chip/Motor temperature exceeds thermal ceiling");

    /* Theorem 3: Swarm Fleet Spatial Separation Non-Aliasing */
    uint64_t fleet_proximity_violation = 0;
    if (fleet != NULL && fleet->robot_count > 1) {
        for (size_t i = 0; i < fleet->robot_count; ++i) {
            for (size_t j = i + 1; j < fleet->robot_count; ++j) {
                double dx = fleet->robots[i].position[0] - fleet->robots[j].position[0];
                double dy = fleet->robots[i].position[1] - fleet->robots[j].position[1];
                double dist = sqrt(dx * dx + dy * dy);
                double min_dist = fleet->robots[i].bounding_radius + fleet->robots[j].bounding_radius;
                if (dist < min_dist) {
                    fleet_proximity_violation = (uint64_t)((min_dist - dist) * 1000.0) + 1;
                    break;
                }
            }
            if (fleet_proximity_violation > 0) break;
        }
    }
    FLOW_SMT_BOX_ADD_RULE(builder, "fleet proximity violation", fleet_proximity_violation, 0, 0,
                          FLOW_BOX_THEOREM_SHARD_ISOLATION, "Robots violate spatial bounding polytope");

    FlowSMTResult res = FLOW_SMT_BOX_VERIFY(builder, "embodied_coordination", proof_out);
    if (res == FLOW_SMT_PROVEN_UNSAT && proof_out != NULL) {
        snprintf(proof_out->proof_summary, sizeof(proof_out->proof_summary),
                 "SMT EMBODIED SOUND: ZMP=stable, Temp=%.1fC <= %.1fC, FleetClearance=sound (Zero-Defect Soundness)",
                 gov ? gov->edge_chip_temp_celsius : 45.0,
                 gov ? gov->thermal_throttle_limit_celsius : 85.0);
    }
    return res;
}
